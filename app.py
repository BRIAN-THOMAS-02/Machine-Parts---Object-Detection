...
def app():
    ...
    You can also refer the graph below where the train_acc, train_loss, vall_acc and val_loss through the epochs can be understood.
    '''
    ...
    if uploaded_file is not None:
        ...
        #39FF14
        if pred[0][0] > 0.5:
            st.write("<p style='font-size: 50px; color: yellow;'> Bolt : {}%  </p>".format(pred[0][0]*100), unsafe_allow_html=True)
        if pred[0][1] > 0.5:
            st.write("<p style='font-size: 50px; color: yellow;'> Locating Pin : {}%  </p>".format(pred[0][1]*100), unsafe_allow_html=True)
        if pred[0][2] > 0.5:
            st.write("<p style='font-size: 50px; color: yellow;'> Nut : {}%  </p>".format(pred[0][2]*100), unsafe_allow_html=True)
        if pred[0][3] > 0.5:
            st.write("<p style='font-size: 50px; color: yellow;'> Washer : {}%  </p>".format(pred[0][3]*100), unsafe_allow_html=True)

        # Create a bar chart to display the prediction probabilities
        #fig = go.Figure([go.Bar(x=class_labels, y=pred_probs)])
        #fig.update_layout(title='Prediction Probabilities', yaxis_title='Probability')
        #st.plotly_chart(fig, use_container_width=True)

        # Show the predicted class label and its probability
        #st.subheader('Prediction')
        #st.write(f'Probability: {pred_probs[class_idx]:.2%}')
